...
contract User {
    ...
    //判断用户地址是否存在
    function isExitUserAddress(address _userAddress) public  returns(bool isIndeed) {
        if (userAddresses.length == 0) return false;
        return (userAddresses[userStruct[_userAddress].index] == _userAddress);
    }

    //判断用户名是否存在
    function isExitUsername(string memory _username) public  returns(bool isIndeed) {
        if (usernames.length == 0) return false;
        return (keccak256(abi.encodePacked(usernames[userListStruct[_username].index])) == keccak256(abi.encodePacked(_username)));
    }

    //根据用户名查找对于的address
    function findUserAddressByUsername(string memory _username) public  returns (address userAddress) {
    ...
    //获取用户个人信息
    function findUser(address _userAddress) public  returns (address  userAddresses, string memory username, uint time, uint index) {
